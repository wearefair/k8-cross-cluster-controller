...
func WithRequestId(ctx context.Context) *zap.Logger {
	return ExistingLoggerWithRequestId(Logger(), ctx)
}

// Returns the provided logger with the request id from the context (x-fair-request-id).
// If no request id exists the field is not added.
func ExistingLoggerWithRequestId(log *zap.Logger, ctx context.Context) *zap.Logger {
	if ctx != nil {
